...
const pkgConfigKey = process.env.PARCEL_NAMER_CUSTOM_STAGE ? `${_package.name}:${process.env.PARCEL_NAMER_CUSTOM_STAGE}` : _package.name; // Plugin configuration importer.
...
const getPluginConfig = projectRoot => {
  ...
  if (projectPackagePromise == null) {
    projectPackagePromise = Promise.resolve(`${(0, _path.join)(projectRoot, 'package.json')}`).then(s => _interopRequireWildcard(require(s))).then(pkgConfig => Object.entries(pkgConfig[pkgConfigKey] || {}).map(([srcPattern, destPattern]) => [new RegExp(srcPattern, 'i'), destPattern]));
  }
